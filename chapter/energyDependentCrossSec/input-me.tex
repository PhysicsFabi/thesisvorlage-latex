...
\chapter{The Energy-Dependence of Inelastic Electron Scattering within the KATRIN \glsentryshort{wgts}}
...
\begin{table}[t]
    \centering
    \xcaption{Inelastic cross section for electrons scattering from molecular hydrogen isotopologues for different incident energies}{Inelastic cross section for electrons scattering from molecular hydrogen isotopologues for different incident energies.}{Listed are important values with reference to KATRIN. Except for the measured value, they are obtained using \eqref{eq:crossSecLiu}. The values are given relative to an assumed endpoint of the measured spectrum $\Eeff=\SI{18575}{eV}$.}
    \begin{tabular}{lll}
        \toprule
